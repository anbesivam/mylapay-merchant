...
export default function RejectedIndividual() {
  ...
  const getLabel = (index) => {
    if (rejectedDocs[index].proofParty) {
      return formik.values[`type_${index}`] + ` (Partner ${index})`;
    } else {
    ...
    validate: (values) => {
      ...
      Object.entries(values).forEach(([k, v]) => {
        ...
        if (
          ...
          values[`type_${curIndex}`] === "Business Address Proof"
        )
          return;
        if (
          k.includes("name") &&
          values[`proof_name_${curIndex}`] === "Registered GSTN Certificate"
        )
          return;
  ...
  const getNumberLabel = (proof_name) => {
    switch (proof_name) {
      ...
      case "Registered GSTN Certificate":
        return "GSTN Number";
