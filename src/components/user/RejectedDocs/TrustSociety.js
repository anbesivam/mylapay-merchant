...
export default function RejectedTrustSociety() {
  ...
  const getLabel = (index) => {
    if (rejectedDocs[index].proofParty) {
      return (
        ...
        ` (trustee ${rejectedDocs[index].proofParty})`
      );
    } else {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      Object.entries(values).forEach(([k, v]) => {
        ...
        if (
          k.includes("name") &&
          (values[`proof_name_${curIndex}`] === "Trust / Society Deed" ||
            values[`proof_name_${curIndex}`] === "Trust Deed" ||
            values[`proof_name_${curIndex}`] === "GSTN" ||
            values[`proof_name_${curIndex}`] ===
              "Board Resolution Stating Opting for IPG Services" ||
            values[`proof_name_${curIndex}`] === "List of Trustee" ||
