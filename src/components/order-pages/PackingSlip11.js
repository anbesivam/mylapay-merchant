import {
  Dialog,
  DialogContent,
  DialogTitle,
  Button,
  Box,
} from "@material-ui/core";
import React, { useState, useEffect } from "react";
import { Grid } from "@material-ui/core";
import { jsPDF } from "jspdf";
import ReactToPrint from "react-to-print";
import { Print, PictureAsPdf } from "@material-ui/icons";

class PackingSlip extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      itemTotal: 12,
      packingSlipData: props.packingSlipData,
      packingSlipDialog: props.packingSlipDialog,
      setPackingSlipDialog: props.setPackingSlipDialog,
      componentRef: props.componentRef,
      orderData: props.orderData,
      orderTotal: props.orderTotal,
    };
  }
  // componentWillUpdate() {
  //   let total_temp = 0.0;
  //   this.state.orderData.map((item) => {
  //     total_temp = Number(total_temp) + Number(item.price * item.quantity);
  //   });
  //   this.setState({
  //     itemTotal: total_temp,
  //   });
  // }
  jsPDFExport() {
    //console.log("jsPDF");
    const input = document.getElementById("pdfTest");
    const pdf = new jsPDF({ unit: "px", format: "letter", userUnit: "px" });

    pdf
      .html(input, {
        html2canvas: { scale: 0.555 },
        margin: [10, 10, 10, 10],
      })
      .then(() => {
        pdf.save(`${this.state.packingSlipData.wbn}`);
        // window.open(pdf.output("bloburl"), "_blank");
      });
  }
  handleDialog() {
    this.setState({
      setPackingSlipDialog: false,
    });
  }

  render() {
    return (
      <>
        <Dialog
          style={{ width: "100%", margin: "auto" }}
          fullWidth
          maxWidth="md"
          open={this.state.packingSlipDialog}
          onClose={this.handleDialog()}
        >
          <DialogTitle id="id">
            <span>Packing Slip</span>
            <span style={{ float: "right" }}>
              <Button
                style={{ marginRight: "10px" }}
                onClick={this.jsPDFExport()}
                variant="contained"
                color="primary"
                startIcon={<PictureAsPdf />}
              >
                Export to PDF
              </Button>

              <Box style={{ display: "inline-block" }}>
                <ReactToPrint
                  trigger={() => (
                    <Button
                      variant="contained"
                      color="primary"
                      endIcon={<Print />}
                    >
                      Click to Print Invoice
                    </Button>
                  )}
                  content={() => this.state.componentRef.current}
                  pageStyle={`
                            @page {
                              size: 210mm 297mm;
                              margin: 10;
                            }
                            
                            @media all {
                              .pagebreak {
                                display: none;
                              }
                            }
                          
                            @media print {
                              .pagebreak {
                                page-break-before: always;
                              }
                            }
                          `}
                />
              </Box>
            </span>
          </DialogTitle>

          <DialogContent>
            <div id="pdfTest">
              <Grid container>
                <Grid item xs={3}></Grid>

                <Grid item xs={6}>
                  <Grid style={{ display: "flex" }}>
                    <Grid
                      item
                      xs={6}
                      style={{
                        border: "1px solid black",
                        textAlign: "center",
                      }}
                    >
                      <div
                        style={{
                          textAlign: "center",
                          marginTop: "5px",
                        }}
                      >
                        <img
                          className="invoice_logo"
                          src={this.state.packingSlipData.delhivery_logo}
                          alt="Mylapay Logo"
                        />
                      </div>
                    </Grid>
                    <Grid
                      item
                      xs={6}
                      style={{
                        border: "1px solid black",
                        textAlign: "center",
                        fontSize: "2em",
                        fontWeight: "bold",
                      }}
                    >
                      <div style={{ marginTop: "5px" }}>
                        <span>
                          <strong>Delhivery</strong>
                        </span>
                      </div>
                    </Grid>
                  </Grid>

                  <Grid
                    item
                    xs={12}
                    style={{
                      textAlign: "center",
                      border: "1px solid black",
                    }}
                  >
                    <div>
                      <img
                        style={{
                          marginTop: "5px",
                          width: "250px",
                          height: "80px",
                        }}
                        className="barcode_img"
                        src={
                          "data:image/png;base64,"
                        }
                        alt="Mylapay Logo"
                        width="100"
                        height="40"
                      />
                    </div>
                    <Grid container style={{ fontSize: "12px" }}>
                      <Grid
                        item
                        xs={6}
                        style={{ display: "flex", justifyContent: "start" }}
                      >
                        <strong>{this.state.packingSlipData.pin}</strong>
                      </Grid>
                      <Grid
                        item
                        xs={6}
                        style={{ display: "flex", justifyContent: "end" }}
                      >
                        <strong>{this.state.packingSlipData.sort_code}</strong>
                      </Grid>
                    </Grid>
                  </Grid>

                  <Grid
                    item
                    xs={12}
                    style={{ display: "flex", fontSize: "12px" }}
                  >
                    <Grid item xs={8} style={{ borderLeft: "1px solid black" }}>
                      <div>
                        <>
                          <span>
                            <strong>Shipping Address:</strong>
                          </span>
                          <br />
                          <span>
                            <strong>
                              {this.state.packingSlipData.name.toUpperCase()}
                            </strong>
                          </span>
                          <br />
                          <span>{this.state.packingSlipData.address}</span>
                          <br />
                          <span>
                            {this.state.packingSlipData.destination_city}
                          </span>
                          <br />
                          <span>
                            {this.state.packingSlipData.st}-
                            {this.state.packingSlipData.pin}
                          </span>
                          <br />
                        </>
                      </div>
                    </Grid>
                    <Grid
                      item
                      xs={4}
                      style={{
                        borderLeft: "1px solid black",
                        borderRight: "1px solid black",
                        textAlign: "center",
                      }}
                    >
                      <div>
                        <>
                          <p style={{ fontWeight: "bold" }}>
                            {this.state.packingSlipData.pt}
                          </p>
                          <p style={{ fontWeight: "bold" }}>
                            {this.state.orderTotal}
                          </p>
                          <p style={{ fontWeight: "bold" }}>{"Express"}</p>
                        </>
                      </div>
                    </Grid>
                  </Grid>
                  <Grid
                    item
                    xs={12}
                    style={{
                      display: "flex",
                      fontSize: "12px",
                      border: "1px solid black",
                    }}
                  >
                    <Grid item xs={8}>
                      <div>
                        <>
                          <span>
                            <strong>
                              Seller:{" "}
                              {this.state.packingSlipData.snm.toUpperCase()}
                            </strong>
                          </span>
                          <br />
                          <span>
                            Address: {this.state.packingSlipData.sadd}
                          </span>
                          <br />
                          <span>{this.state.packingSlipData.origin_city}</span>
                        </>
                      </div>
                    </Grid>
                  </Grid>
                  <Grid
                    item
                    xs={12}
                    style={{ display: "flex", borderBottom: "1px solid black" }}
                  >
                    <Grid
                      item
                      xs={8}
                      style={{
                        borderLeft: "1px solid black",
                        textAlign: "center",
                        fontWeight: "bold",
                      }}
                    >
                      <div>
                        <>
                          <p>Product</p>
                        </>
                      </div>
                    </Grid>
                    <Grid
                      item
                      xs={2}
                      style={{
                        borderLeft: "1px solid black",
                        textAlign: "center",
                        fontWeight: "bold",
                      }}
                    >
                      <div>
                        <>
                          <p style={{}}>Price </p>
                        </>
                      </div>
                    </Grid>
                    <Grid
                      item
                      xs={2}
                      style={{
                        borderLeft: "1px solid black",
                        borderRight: "1px solid black",
                        textAlign: "center",
                      }}
                    >
                      <div>
                        <>
                          <p style={{ fontWeight: "bold" }}>Total </p>
                        </>
                      </div>
                    </Grid>
                  </Grid>

                  {this.state.orderData.map((item) => {
                    return (
                      <>
                        <Grid item xs={12} style={{ display: "flex" }}>
                          <Grid
                            item
                            xs={8}
                            style={{
                              borderLeft: "1px solid black",
                              textAlign: "center",
                              padding: "5px",
                            }}
                          >
                            <div>
                              <>
                                <span>{item.item}</span>
                              </>
                            </div>
                          </Grid>
                          <Grid
                            item
                            xs={2}
                            style={{
                              borderLeft: "1px solid black",
                              textAlign: "center",
                              padding: "5px",
                            }}
                          >
                            <div>
                              <>
                                <span>{item.price} </span>
                              </>
                            </div>
                          </Grid>
                          <Grid
                            item
                            xs={2}
                            style={{
                              borderLeft: "1px solid black",
                              borderRight: "1px solid black",
                              textAlign: "center",
                              padding: "5px",
                            }}
                          >
                            <div>
                              <>
                                <span>{item.itemTotal_amount} </span>
                              </>
                            </div>
                          </Grid>
                        </Grid>
                      </>
                    );
                  })}
                  <Grid
                    item
                    xs={12}
                    style={{ display: "flex", border: "1px solid black" }}
                  >
                    <Grid
                      item
                      xs={10}
                      style={{ textAlign: "center", fontWeight: "bold" }}
                    >
                      <div>
                        <>
                          <p>Total</p>
                        </>
                      </div>
                    </Grid>

                    <Grid
                      item
                      xs={2}
                      style={{
                        borderLeft: "1px solid black",
                        textAlign: "center",
                      }}
                    >
                      <div>
                        <>
                          <p style={{ fontWeight: "bold" }}>
                            {this.state.itemTotal.toFixed(2)}
                          </p>
                        </>
                      </div>
                    </Grid>
                  </Grid>
                  <Grid
                    item
                    xs={12}
                    style={{
                      textAlign: "center",
                      borderLeft: "1px solid black",
                      borderRight: "1px solid black",
                    }}
                  >
                    <div>
                      <img
                        style={{
                          marginTop: "5px",
                          width: "250px",
                          height: "80px",
                        }}
                        className="barcode_img"
                        src={
                          "data:image/png;base64,"
                        }
                        alt="Mylapay Logo"
                        width="100"
                        height="40"
                      />
                    </div>
                  </Grid>
                  <Grid
                    item
                    xs={12}
                    style={{
                      border: "1px solid black",
                      fontSize: "12px",
                    }}
                  >
                    <Grid
                      style={{
                        display: "flex",
                        fontSize: "12px",
                      }}
                    >
                      <div>
                        <>
                          <span>
                            <strong>Return Address:</strong>
                          </span>
                          <br />
                          <span>
                            <strong>
                              {this.state.packingSlipData.snm.toUpperCase()}
                            </strong>
                          </span>
                          <br />
                          <span>{this.state.packingSlipData.sadd}</span>
                          <br />
                          <span>{this.state.packingSlipData.origin_city}</span>
                          <br />
                          <span>
                            {this.state.packingSlipData.rst}-
                            {this.state.packingSlipData.rpin}
                          </span>
                        </>
                      </div>
                    </Grid>
                  </Grid>
                </Grid>

                <Grid item xs={3}></Grid>
              </Grid>
            </div>
          </DialogContent>
        </Dialog>
      </>
    );
  }
}
export default PackingSlip;
