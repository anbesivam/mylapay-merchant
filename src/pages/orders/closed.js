...
export default function Closed() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Order Time",
      options: {
        setCellProps: () => ({ style: { minWidth: "30px", maxWidth: "30px" }}),
        sort: true,
  ...
  const handleRowClick = (rowData, rowMeta) => {
    history.push(`/orders/transitvieworder?id=${rowData[3]}&orderStatus=4`);
  };
  ...
  const options = {
    ...
    download: true,
    downloadOptions: {
      filename: `Closed_Orders_${moment().format("YYYY-MM-DD")}.csv`,
    },
    viewColumns: false,
    print: false,
    elevation: 0,
  ...
  return (
    ...
    >
      <MUIDataTable
        className="transanctions-table-closed"
