...
export default function Transit() {
  ...
  const cancelOrder = async () => {
    if (selecteddelivery == "selfpickup") {
      ...
      cancelDunzoShipment(2);
    } else if (selecteddelivery == "delhivery") {
      cancelDelhiveryShipment(2);
    }
    // let awb_code = [];
  ...
  const cancelDunzoShipment = async (cancelType) => {
    let order_id;
    if (rowsSelectedval.length > 0) {
      rowsSelectedval.filter((item, index) => {
        if (data[item].iOrder) {
  ...
  const cancelDelhiveryShipment = async (cancelType) => {
    let order_id;
    if (rowsSelectedval.length > 0) {
      rowsSelectedval.filter((item, index) => {
        if (data[item].iOrder) {
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            axios
              .post(
                `${REACT_APP_DELHIVERY_URL}/mylapay/delhivery/cancel/order`,
                {
                  delhivery: order_id,
                  cancellation_reason: reason,
                  cancelType: cancelType,
  ...
  const cancelShipment = async (cancel_Type) => {
    let awb_code = [];
    let door_delivery_id = [];
    let order_id;
    if (rowsSelectedval.length > 0) {
