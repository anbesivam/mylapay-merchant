...
export default function YourCustomers() {
  ...
  const handleRowClick = (rowData, rowMeta) => {
    console.log(rowData);
      //`http://localhost:3000/your-customers/payment-webpage-breakup?id=${rowData[2]}`
    window.open(
      `${REACT_APP_MERCHANT_URL}/your-customers/payment-webpage-breakup?id=${rowData[2]}`
    );
  };
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      await axios
        .get("/mylapay/transaction/get/customer_list_webpage")
        .then((response) => {
  ...
  return (
    ...
    >
      <MUIDataTable
        ...
        title="Payment Webpage Customers"
        // title={
        //   <Button
        //     variant="contained"
        //     size="medium"
